...
function join(c11: Matrix, c12: Matrix, c21: Matrix, c22: Matrix, h: number): Matrix {
  ...
  for (let i=0;i<h;i++){for(let j=0;j<h;j++){C[i]![j]=c11[i]![j]!;C[i]![j+h]=c12[i]![j]!;}}
  for (let i=0;i<h;i++){for(let j=0;j<h;j++){C[i+h]![j]=c21[i]![j]!;C[i+h]![j+h]=c22[i]![j]!;}}
  return C;
}

/** Strassen matrix multiplication O(n^2.807) */
export function strassenMul(A: Matrix, B: Matrix): Matrix { return strassen(A, B); }
