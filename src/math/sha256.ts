...
// DEPRECATED (v0.1.1): cryptographic primitives move to `lombokencryptdecrypt` (ADR-016);
// this module is removed in v0.2.0.
...
function processBlock(state: Uint32Array, block: Uint32Array): void {
  ...
  for (let i = 16; i < 64; i++) {
    const s0 =
      rotr32(W[i - 15] as number, 7) ^
      rotr32(W[i - 15] as number, 18) ^
      ((W[i - 15] as number) >>> 3);
    const s1 =
      rotr32(W[i - 2] as number, 17) ^
      rotr32(W[i - 2] as number, 19) ^
      ((W[i - 2] as number) >>> 10);
...
/**
 * SHA-256 hash (FIPS 180-4). Returns a 32-byte Uint8Array.
...
export function hmacSha256(key: Uint8Array, data: Uint8Array): Uint8Array {
  ...
  for (let i = 0; i < blockSize; i++) {
    ipad[i] = (kPadded[i] as number) ^ 0x36;
    opad[i] = (kPadded[i] as number) ^ 0x5c;
  }
