...
import { l2Distance } from './similarity.js';
...
export function kmeans(
  ...
  for (; iter < maxIter; iter++) {
    ...
    for (let i = 0; i < n; i++) {
      for (let j = 0; j < d; j++) newCentroids[newLabels[i]!]![j]! += points[i]![j] as number;
      counts[newLabels[i]!]!++;
    }
    let maxShift = 0;
    for (let c = 0; c < k; c++) {
      ...
      for (let j = 0; j < d; j++) newCentroids[c]![j]! /= counts[c] as number;
