...
export function randomBytes(n: number): Uint8Array {
  ...
  if (c === undefined) {
    throw new Error('randomBytes: globalThis.crypto.getRandomValues is unavailable');
  }
...
export class Xoshiro256pp {
  ...
  nextInt(n: number): number {
    if (!Number.isInteger(n) || n < 1 || n > Number.MAX_SAFE_INTEGER) {
      throw new RangeError('nextInt: n must be an integer in [1, 2^53)');
    }
    const bn = BigInt(n);
...
export class Pcg32 {
  ...
  nextBounded(bound: number): number {
    if (!Number.isInteger(bound) || bound < 1 || bound > 0xffffffff) {
      throw new RangeError('nextBounded: bound must be an integer in [1, 2^32)');
    }
