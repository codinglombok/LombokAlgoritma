...
import { polynomialHash } from '../src/string/hash/index.js';
...
describe('core/numeric + safe-int + errors + types', () => {
  it('numeric', () => {
    ...
    expect(() => isqrt(-1)).toThrow(AlgoError);
    expect(ceilDiv(7, 2)).toBe(4);
    expect(alignUp(13, 8)).toBe(16);
    expect(() => ceilDiv(1, 0)).toThrow(AlgoError);
  });
  it('safe-int', () => {
  ...
  it('errors carry codes', () => {
    ...
    expect(new AlgoError('INVALID_INPUT', 'm', 'cause').cause).toBe('cause');
  });
  it('types helpers', () => {
...
describe('math — number theory', () => {
  ...
  it('pollardRho returns a proper factor and rejects primes', () => {
    ...
    expect(() => pollardRho(97n)).toThrow(AlgoError);
    expect(() => pollardRho(3n)).toThrow(AlgoError);
  });
});
...
describe('math — transforms and matrices', () => {
  ...
  it('ntt round-trip and polynomial multiplication', () => {
    ...
    expect(() => ntt([1n, 2n, 3n])).toThrow(AlgoError);
    expect(() => ntt([1n, 2n], 7n)).not.toThrow();
    expect(() => ntt([1n, 2n, 3n, 4n], 7n)).toThrow(AlgoError);
  });
  it('matrix ops and Strassen on non-power-of-two sizes', () => {
    ...
    expect(() => matMul([[1, 2]], [[1, 2]])).toThrow(AlgoError);
    ...
    expect(() => strassenMul([[1, 2]], [[1, 2]])).toThrow(AlgoError);
  });
});
