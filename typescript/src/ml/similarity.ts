...
/** Cosine similarity dot(a, b) / (‖a‖·‖b‖); 0 when either vector is zero. */
...
/**
 * Cosine similarity of `query` against every candidate, sorted by score descending, ties by index
 * ascending (SPEC §9.3). Zero-norm vectors score 0 (v0.1.x produced NaN, which made the sort order
 * engine-dependent). Each score is computed exactly as {@link cosineSimilarity}.
 */
export function batchCosine(
  ...
  return candidates
    .map((c, i) => ({ index: i, score: cosineSimilarity(query, c) }))
