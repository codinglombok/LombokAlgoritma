...
import { quicksort, timsort, mergesort, heapsort, radixSortLSD, countingSort } from '../../src/sort/index.js';
...
describe('Sort — correctness', () => {
  for (const { name, fn } of ALGOS) {
    describe(name, () => {
      ...
      it('single element', () => { expect(fn([42])).toEqual([42]); });
      it('already sorted', () => { expect(fn([1,2,3,4,5])).toEqual([1,2,3,4,5]); });
      it('reverse sorted', () => { expect(fn([5,4,3,2,1])).toEqual([1,2,3,4,5]); });
      it('duplicates', () => { expect(fn([3,1,2,1,3])).toEqual([1,1,2,3,3]); });
      it('all identical', () => { expect(fn([7,7,7,7])).toEqual([7,7,7,7]); });
      it('two elements', () => { expect(fn([2,1])).toEqual([1,2]); });
      it('negative numbers', () => { expect(fn([-3,-1,-2])).toEqual([-3,-2,-1]); });
      it('mixed sign', () => { expect(fn([3,-1,0,2,-2])).toEqual([-2,-1,0,2,3]); });
  ...
  describe('timsort — stability', () => {
    it('preserves relative order of equal elements', () => {
      const arr = [
        { v: 1, i: 0 }, { v: 2, i: 1 }, { v: 1, i: 2 }, { v: 2, i: 3 },
      ];
  ...
  describe('radixSortLSD', () => {
    it('empty', () => { expect(radixSortLSD([])).toEqual([]); });
    it('positive integers', () => { expect(radixSortLSD([170,45,75,90,802,24,2,66])).toEqual([2,24,45,66,75,90,170,802]); });
    it('with negatives', () => { expect(radixSortLSD([-3,-1,0,2])).toEqual([-3,-1,0,2]); });
  });

  describe('countingSort', () => {
    it('basic', () => { expect(countingSort([3,1,2,1,3,0])).toEqual([0,1,1,2,3,3]); });
    it('all zeros', () => { expect(countingSort([0,0,0])).toEqual([0,0,0]); });
  });
});
...
describe('Sort — custom comparator', () => {
  it('sort strings by length', () => {
    ...
    expect(result.map(s => s.length)).toEqual([3, 3, 5, 6]);
  });
