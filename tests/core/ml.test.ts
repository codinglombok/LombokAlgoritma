...
import {
  cosineSimilarity, l2Distance, l1Distance,
  dotProduct, normalize, batchCosine, pearson,
  jaccardSimilarity,
...
describe('l2Distance', () => {
  it('same vector = 0', () => { expect(l2Distance([1,2],[1,2])).toBe(0); });
  it('known value', () => { expect(l2Distance([0,0],[3,4])).toBeCloseTo(5); });
...
describe('l1Distance', () => {
  it('basic', () => { expect(l1Distance([1,2,3],[4,5,6])).toBe(9); });
  it('same = 0', () => { expect(l1Distance([1,2],[1,2])).toBe(0); });
});

describe('dotProduct', () => {
  it('known value', () => { expect(dotProduct([1,2,3],[4,5,6])).toBe(32); });
  it('zero', () => { expect(dotProduct([1,0],[0,1])).toBe(0); });
});
...
describe('batchCosine', () => {
  it('returns sorted by score descending', () => {
    const q = [1,0];
    const candidates = [[0,1],[-1,0],[1,0],[0.7,0.7]];
...
describe('pearson', () => {
  ...
  it('no correlation', () => {
    // approximately 0 for uncorrelated
...
describe('k-Means', () => {
  it('clusters 2D points', () => {
    ...
    const cluster2 = Array.from({length:20}, () => [10+Math.random()*0.5, 10+Math.random()*0.5]);
