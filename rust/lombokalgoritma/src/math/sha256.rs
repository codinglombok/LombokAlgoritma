//! SHA-256 (FIPS 180-4).
//!
//! **Deprecated:** cryptographic primitives move to `lombokencryptdecrypt`; this module is
//! removed in v0.2.0.
#![allow(deprecated)]
...
#[deprecated(
    since = "0.1.1",
    note = "moved to `lombokencryptdecrypt`; removed in v0.2.0"
)]
pub fn sha256(msg: &[u8]) -> [u8; 32] {
    let bit_len = (msg.len() as u64).wrapping_mul(8);
    let pad_len = (msg.len() + 9).div_ceil(64) * 64;
    let mut padded = vec![0u8; pad_len];
    padded[..msg.len()].copy_from_slice(msg);
    padded[msg.len()] = 0x80;
    padded[pad_len - 8..].copy_from_slice(&bit_len.to_be_bytes());
    let mut state = IV;
...
/// SHA-256 digest of `msg` as lowercase hex.
#[deprecated(
    since = "0.1.1",
    note = "moved to `lombokencryptdecrypt`; removed in v0.2.0"
)]
