# LombokAlgoritma — Python regression & coverage tests (v0.1.1)
...
def test_sha256_fips_180_4() -> None:
    assert (
        math.sha256_hex("abc") == "ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad"
    )
    assert (
        math.sha256_hex("abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq")
        == "248d6a61d20638b8e5c026930c3e6039a33ce45964ff2167f6ecedd419db06c1"
    )
    assert (
        math.sha256_hex(b"a" * 1_000_000)
        == "cdc76e5c9914fb9281a1c7e284d73e67f1809a48a497200e046d39ccc7112cd0"
    )
    assert len(math.sha256("x")) == 32


def test_crypto_is_deprecated() -> None:
    with pytest.deprecated_call():
        math.sha256(b"")
    with pytest.deprecated_call():
        math.hmac_sha256(b"k", b"d")
...
def test_hkdf_rfc5869_case1() -> None:
    ...
    with pytest.raises(ValueError):
        math.hkdf(b"x", 255 * 32 + 1)


@pytest.mark.filterwarnings("ignore::DeprecationWarning")
def test_hmac_rfc4231_case2() -> None:
    assert (
        math.hmac_sha256(b"Jefe", b"what do ya want for nothing?").hex()
...
def test_number_theory() -> None:
    ...
    assert math.mod_inverse(2, 4) is None and math.mod_inverse(-3, 11) == 7
